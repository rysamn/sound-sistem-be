...
public class JobOrderDao {
    ...
    private final RowMapper<JobOrderDto> M = new RowMapper<>() {
        ...
        public JobOrderDto mapRow(ResultSet rs, int rowNum) throws SQLException {
            ...
            if(t1 != null) j.setTanggalMulai(t1.toLocalDateTime());
            java.sql.Timestamp t2 = rs.getTimestamp("tanggal_selesai");
            if(t2 != null) j.setTanggalSelesai(t2.toLocalDateTime());
    ...
    public List<JobOrderDto> findAll(){ return jdbc.query("SELECT * FROM job_order ORDER BY id", M); }
    public JobOrderDto findById(int id){ return jdbc.queryForObject("SELECT * FROM job_order WHERE id=?", M, id); }
    public int insert(JobOrderDto j){
        return jdbc.update("INSERT INTO job_order (nama_penyewa,venue,acara,tanggal_mulai,tanggal_selesai,harga_sewa,status_pembayaran,metode_bayar,no_rekening,tanggal_order) VALUES (?,?,?,?,?,?,?,?,?,?)",
    ...
    public int update(JobOrderDto j){
        return jdbc.update("UPDATE job_order SET nama_penyewa=?, venue=?, acara=?, tanggal_mulai=?, tanggal_selesai=?, harga_sewa=?, status_pembayaran=?, metode_bayar=?, no_rekening=?, tanggal_order=? WHERE id=?",
                j.getNamaPenyewa(), j.getVenue(), j.getAcara(),
                j.getTanggalMulai()==null?null:java.sql.Timestamp.valueOf(j.getTanggalMulai()),
                j.getTanggalSelesai()==null?null:java.sql.Timestamp.valueOf(j.getTanggalSelesai()),
                j.getHargaSewa(), j.getStatusPembayaran(), j.getMetodeBayar(), j.getNoRekening(), j.getTanggalOrder(), j.getId());
    }
    public int delete(int id){ return jdbc.update("DELETE FROM job_order WHERE id=?", id); }
}
